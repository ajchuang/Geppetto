...
void init (int argc, char** argv) {
    ros::init (argc, argv, "left_listener");   
...
void chatterCallback (const sensor_msgs::JointState & joint){
    int i;
    int joints[7] = {32,33,31,35,34,36,37};
    for (i=0; i<7; i++){
...
pr2_controllers_msgs::JointTrajectoryGoal armExtensionTrajectory (double* data){    
    ...
    ind += 1;
    goal.trajectory.points[ind].positions.resize(7);
    goal.trajectory.points[ind].positions[0] = data[7];
    goal.trajectory.points[ind].positions[1] = data[8];
    goal.trajectory.points[ind].positions[2] = data[9];
    goal.trajectory.points[ind].positions[3] = data[10];
    goal.trajectory.points[ind].positions[4] = data[11];
    goal.trajectory.points[ind].positions[5] = data[12];
    goal.trajectory.points[ind].positions[6] = data[13];
